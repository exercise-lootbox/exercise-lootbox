...
function Home() {
  ...
  function stravaActivity(activity: any) {
    return (
      <>
        ...
        <div className="activity-subtitle">
          {`⛰️ Elevation: ${activity.elevation} meters`}
        </div>
      </>
    );
  }
  ...
  function homeContent(stravaData: any) {
    return (
      <div>
        <h3>{isLoggedIn ? `Welcome back, ${userInfo.firstName}!` : "Welcome to FitCoin!"}</h3>
        ...
        {(stravaId !== "" || !isLoggedIn) &&
          <div>
            {!isLoggedIn &&
              <h4 className="sample-data-title">This is just sample data until you log in!</h4>}
